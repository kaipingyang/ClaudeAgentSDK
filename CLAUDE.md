...
exported/documented topic to appear in it. When you add a new exported function or type
(a new `man/*.Rd`), either add it to the appropriate `reference:` section in
`_pkgdown.yml` or mark it `@keywords internal` — otherwise the pkgdown deploy fails
("Reference metadata not ok / N topics missing from index") on every push to `main`.
